...
int physicalMemory[128][256]; //physical memory size=(number of frames)*(size of frames)
int pf = 0;   //page faults counter
int TLB_hits = 0;      //TLB hit counter
int dirty = 175; // dirty counter
...
void read_padrs(l_adrs,xd){
    ...
    if (xd == 1)
    {
    	strcpy(value_c,"dirty");
    	printf("Virtual address: %d Physical address: %d Value: %c\n", l_adrs, (frameNumber << 8) | offset, value_c);
    }
    else{
...
int main(int argc, char* argv[])
{
	...
	read_padrs(l_adrs,xd); //read from physical memory using logical address
	xd = 0;
	...
	printf("Total number of dirty bits: %d\n",dirty);
